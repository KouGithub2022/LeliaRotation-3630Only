...
public sealed class RDMPvE_Default : RDM_Base
{
    public override CombatType Type => CombatType.PvE;

    public override string GameVersion => "6.31";

    public override string RotationName => "Lelia Standard";
	...
	protected override IRotationConfigSet CreateConfiguration() => base.CreateConfiguration()
        .SetCombo(CombatType.PvE, "EngagementCh", 0, "デプラスマン＆アンガジェマンの残チャージ数。", "0", "1", "2")
	...
	protected override bool EmergencyAbility(IAction nextGCD, out IAction act)
	{
		...
		if (Scorch.CanUse(out act)) return false;
		if (Resolution.CanUse(out act)) return false;

		if (Configs.GetBool("UseAcceleration") && Acceleration.CurrentCharges == 2 &&
			Acceleration.CanUse(out act)) return true;

		if (IsBurst && HasHostilesInRange && Embolden.CanUse(out act, CanUseOption.MustUse)) return true;

		if (((Player.HasStatus(true, StatusID.Embolden) || IsLastAbility(ActionID.Embolden)) && ManaStacks == 0 && WhiteMana <= 50 && BlackMana <= 50) &&
	...
	protected override bool AttackAbility(out IAction act)
	{
		...
		if (ManaStacks == 0 && (BlackMana < 50 || WhiteMana < 50)
			&& (CombatElapsedLess(4) || !Manafication.EnoughLevel ||
				!Manafication.WillHaveOneChargeGCD(0, 1)))
		{
		...
		if (Engagement.CurrentCharges > (Configs.GetCombo("EngagementCh")) && Engagement.CanUse(out act, CanUseOption.EmptyOrSkipCombo)) return true;
